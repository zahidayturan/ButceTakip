        ...
        child: Row(
          ...
          children: [
                  ...
                  GestureDetector(
                    onTap: () {
                      if(widget.addDataMode == 0){
                        ...
                        readAdd.resetControllerForUpdateOrAgain(context);
                        setState(() {
                    ...
                    child: Container(
                      ...
                      child: Center(
                        child: Text( widget.addDataMode == 0 ? translation(context).deleteAll : "Geri Al",
                            style: TextStyle(
                                height: 1,
            ...
            SizedBox(
              ...
              child:widget.addDataMode == 0 ? GestureDetector(
                onTap: ()  async{
                ...
                child: Container(
                  ...
                  child: TextButton(
                    onPressed: ()  {
                      ...
                      double amount = double.tryParse(readAdd.amount.text) ?? 0.0;
                      if (amount != 0.0 && readAdd.category.text.isNotEmpty) {
                        if (_isProcessOnceValidNumber(readAdd.customize.text) == true &&
                            readAdd.customize.text != "") {
                          List <String> parts = readAdd.customize.text.split("/");
                          readAdd.customize.text = parts.length == 2 ?  parts[1] : parts[0];
                          amount = double.parse((amount / double.parse(readAdd.customize.text)).toStringAsFixed(2));
                          readAdd.systemMessageController.text = "1/${readAdd.customize.text}";
                          readAdd.convertedCustomize = "1/${readAdd.customize.text}";
                          readAdd.customize.text = "1/${readAdd.customize.text}";
                        } else if (_isProcessOnceValidNumber(readAdd.customize.text) == false &&
                            readAdd.customize.text != "") {
                          readAdd.systemMessage = readAdd.convertedCustomize != "" ? readAdd.convertedCustomize : Converter().textConverterToDB(readAdd.customize.text,context,1);
                        }
                        if(widget.addDataMode == 1){
                          readUpdateData.updateDataBase(
                              int.parse(readAdd.updateOrAgainItemsId!),
                              readAdd.operationType.text,
                              readAdd.convertedCategory,
                              readAdd.operationTool.text,
                              int.parse(readAdd.registration.text),
                              ...
                              readAdd.operationDate.text,
                              readAdd.moneyType.text,
                              readAdd.convertedCustomize != "" ? readAdd.convertedCustomize : Converter().textConverterToDB(readAdd.customize.text,context,1),
                              ref.read(currencyRiverpod).calculateRealAmount(amount, readAdd.moneyType.text, ref.read(settingsRiverpod).Prefix!),
                              readAdd.userCategoryController != "" ? readAdd.userCategoryController : readAdd.category.text == readAdd.userCategory.text ? readAdd.userCategory.text : "",
                              readAdd.customize.text != "" ? readAdd.systemMessage.toString() : "");
                          read.update();}
                        else{
                          read.insertDataBase(
                            readAdd.operationType.text,
                        ...
                        readHome.setStatus();
                        readDailyInfo.setSpendDetailItemWithId(int.parse(readAdd.updateOrAgainItemsId!));
                        if (adCounter == 0) {
                          _showInterstitialAd(context);
                          readSettings.resetAdCounter();
                        } else {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            ...
                            behavior: SnackBarBehavior.floating,
                            content: Text(
                              widget.addDataMode == 1 ? translation(context).activityUpdated : translation(context).activityAdded,
                              style: TextStyle(
                                color: renkler.yaziRenk,
                                fontSize: 16,
                                fontFamily: 'Nexa3',
                    ...
                    child: Text(widget.addDataMode == 1 ? translation(context).updateDone : translation(context).done,
