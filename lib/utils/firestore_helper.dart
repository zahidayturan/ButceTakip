...
class firestoreHelper {
  ...
  static void createHistoryRates(currencyInfo info) async {
    ...
    final docCurrency = await FirebaseFirestore.instance
        .collection("historyRates")
        .doc(formattedDate);
    final json = info.toMap();
    await docCurrency.set(json);
  }

  static Future<List<currencyInfo>> getHistoryCurrency(
      {int? day, int? month, int? year}) async {
    currencyInfo currency;
    final collectionReference =
        FirebaseFirestore.instance.collection("historyRates");
    ...
    currenciesList.sort((a, b) => DateTime.tryParse(b.lastApiUpdateDate!)!
        .compareTo(DateTime.tryParse(a.lastApiUpdateDate!)!));
  ...
  static void searchHistoryRate(
      String formattedDatee, currencyInfo info) async {
    final collectionReference =
        FirebaseFirestore.instance.collection("historyRates");
    QuerySnapshot querySnapshot = await collectionReference
        .where(FieldPath.documentId, isEqualTo: formattedDatee)
        .get();
  ...
  static void createCurrencyFirestore(currencyInfo info) async {
    final docCurrency =
        await FirebaseFirestore.instance.collection("rates").doc("currencies");
    final json = info.toMap();
    await docCurrency.set(json);
  }
